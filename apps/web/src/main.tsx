...
function Dashboard({ email, systemRole }: { email: string; systemRole: "user" | "superadmin" }) {
  ...
  useEffect(() => {
    ...
    if (activeView === "admin" && systemRole === "superadmin") {
      apiRequest<AdminOverview>("/api/admin/overview", { signal: controller.signal })
...
function UserLocationMap({ locations, activeUsers }: { locations: AdminOverview["locations"]; activeUsers: number }) {
  ...
  const point = (item: AdminOverview["locations"][number]) => ({ x: maxLng === minLng ? 50 : 8 + ((item.longitude - minLng) / (maxLng - minLng)) * 84, y: maxLat === minLat ? 30 : 8 + ((maxLat - item.latitude) / (maxLat - minLat)) * 44 });
  return <article className="panel locationPanel"><div className="panelHeaderRow"><div><div className="panelKicker">Field presence</div><h2>User activity map</h2></div><span className="countPill">{activeUsers} reported</span></div><p className="mapDisclaimer">Locations represent each user&apos;s latest verified field submission, not live tracking.</p><div className="locationMap"><svg viewBox="0 0 100 60" role="img" aria-label={`Map showing ${locations.length} latest field locations`} preserveAspectRatio="none"><path d="M0 15H100M0 30H100M0 45H100M25 0V60M50 0V60M75 0V60" /><text x="3" y="7">N</text>{locations.map((item) => { const marker = point(item); return <g key={item.user_id} transform={`translate(${marker.x} ${marker.y})`}><title>{`${item.full_name || item.email || "User"}, ${item.farm_name}`}</title><circle r="3.1" /><circle className="mapPointCore" r="1.2" /></g>; })}</svg></div><div className="locationList">{locations.map((item) => <div key={item.user_id}><strong>{item.full_name || item.email || "Unnamed user"}</strong><span>{item.farm_name} · {item.latitude.toFixed(4)}, {item.longitude.toFixed(4)} · {new Date(item.captured_at).toLocaleString()}</span></div>)}</div></article>;
}
